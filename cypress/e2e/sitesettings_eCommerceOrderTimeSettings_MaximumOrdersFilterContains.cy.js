describe('Setting a Contain filter condition uner Maximum Order column', () => {
    it('should display only list where Maximum Orders column contain specified number in the filter condition', () => {
      ...
      cy.wait(2000);
      cy.get('#AvailableOrdersoption').select('Contains');
      cy.get('#AvailableOrderstxt').type('5');
      cy.wait(2000);
      cy.get('button.dashboardbtn.filterbtn').eq(7).click();
      cy.get('span.field-val') .should('contain.text', '5');
