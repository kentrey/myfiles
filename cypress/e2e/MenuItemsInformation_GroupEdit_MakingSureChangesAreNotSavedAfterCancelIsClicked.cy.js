describe('Group Edit General tab  Rechecking if changes made are NOT saved per menu item when Clicked Cancel spec', () => {
    it('should NOT reflect the changes made in the General Information tab for each Menu items', () => {
        ...
        cy.get('#DepartmentField_FieldValue').select('SOUP');
        ...
        cy.wait(2000);
        cy.get('div.field-col select#DepartmentListText').should('not.have.value', '421BB7A1-1686-EC11-B867-001A7DDA7113');
        cy.get('input#dis_18').should('not.be.checked');
        cy.get('div.btnstab-wrap a[href="/meal/item/Info"]').eq(0).click();
        cy.wait(2000);
        cy.get('input.search-people').type('XXX-TEST UPDATE');
        cy.wait(2000);
