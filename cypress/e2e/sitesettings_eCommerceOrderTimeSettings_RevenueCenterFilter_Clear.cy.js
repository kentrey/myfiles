describe('Setting a filter condition under Revenue Center column but decided to clear it', () => {
    it('should still display the entire list in the eCommerce Order Time Settings table', () => {
      ...
      cy.get('span.filter').eq(9).click();
      cy.wait(2000);
      cy.get('button.dashboardbtn.clearfilter').eq(9).click();
      cy.get('#RevenueCentertxt').should('not.be.visible');
