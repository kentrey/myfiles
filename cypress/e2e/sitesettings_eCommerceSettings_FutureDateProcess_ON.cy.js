describe('Turning ON the Future Date Process spec', () => {
    it('should turn ON the Future Date Process', () => {
      ...
      cy.get('#IsFutureDateOnOff').invoke('prop', 'checked').then((isChecked) => {
        ...
        if (isChecked) {
          ...
          // If checkbox is currently 'OFF', check and save it
          cy.get('#IsFutureDateOnOff').check();
          cy.get('button.dashboardbtn.ecom-settings').click();
          
        }
      })
      cy.get('#IsFutureDateOnOff').should('be.checked');
